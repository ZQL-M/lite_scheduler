...
(
    ...
    `job_class`     VARCHAR(255)          DEFAULT NULL COMMENT '任务类名（用于 TubaJobFactory 创建实例）',
    ...
    `timeout`       BIGINT                DEFAULT NULL COMMENT '任务执行超时时间（毫秒）',
    `retry_count`   INT          NOT NULL DEFAULT 0 COMMENT '任务失败重试次数',
    `retry_interval` BIGINT                DEFAULT NULL COMMENT '任务失败重试间隔（毫秒）',
