-- Lite-Scheduler 数据库建表脚本

-- 1. 任务定义表：存储任务的基本信息和执行目标
CREATE TABLE IF NOT EXISTS `task_definition` (
  ...
  `method_name` VARCHAR(128) NOT NULL COMMENT '执行方法名称',
...
CREATE TABLE IF NOT EXISTS `task_cron_trigger` (
  ...
  FOREIGN KEY (`trigger_id`) REFERENCES `task_trigger` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Cron触发器表';

-- 4. 简单触发器表：存储固定频率和固定延迟类型的触发器配置
...
CREATE TABLE IF NOT EXISTS `task_status` (
  ...
  PRIMARY KEY (`task_id`),
  FOREIGN KEY (`task_id`) REFERENCES `task_definition` (`id`) ON DELETE CASCADE,
  INDEX `idx_status_next_fire_time` (`next_fire_time`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='任务状态表';

-- 6. 任务执行日志表：记录任务的每次执行历史
CREATE TABLE IF NOT EXISTS `task_execution_log` (
  ...
  PRIMARY KEY (`id`),
  FOREIGN KEY (`task_id`) REFERENCES `task_definition` (`id`) ON DELETE CASCADE,
  FOREIGN KEY (`trigger_id`) REFERENCES `task_trigger` (`id`) ON DELETE SET NULL,
  INDEX `idx_log_task` (`task_id`),
  INDEX `idx_log_trigger` (`trigger_id`),
